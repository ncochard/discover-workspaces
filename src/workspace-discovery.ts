...
import {
    ...
    PackageIdentifier,
    PackageTypes,
    WorkspaceDiscoveryDependencies,
} from "./types";

export const workspaceDiscovery =
    (dependencies: WorkspaceDiscoveryDependencies) =>
    async (input: DiscoverWorkspacesInput): Promise<PackageDetails[]> => {
        const rootPkg = await readPackageJson(dependencies)(input.cwd);
        if (!rootPkg) {
            return [];
        }
        const globs = rootPkg.packageJson.workspaces || [];
        const folders = mergeWorkspacePaths(await expendGlobs(input.cwd, globs));
        const packages: PackageDetails<PackageIdentifier>[] = await readPackagesJson(dependencies)(folders);
